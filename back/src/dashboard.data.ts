...
export async function getTopCountryByGame() {
  return await execute<{ game: string; countries: string[] }[]>(
    dashboardQueries.gameSalesByPlatform,
    []
  );
}
...
export async function getTopSellingGamesEachFiveYears() {
  return await execute<{ years: string; game: string; totalSales: number }[]>(
    dashboardQueries.gameSalesByPlatform,
    []
  );
}
...
export async function getBestRatedGames() {
  return await execute<{ game: string; rating: number }[]>(
    dashboardQueries.gameSalesByPlatform,
    []
  );
}
